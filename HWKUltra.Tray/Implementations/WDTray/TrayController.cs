...
namespace HWKUltra.Tray.Implementations.WDTray
{
    ...
    public class TrayController : ITrayController
    {
        ...
        private readonly Dictionary<string, SlotStateDefinition> _slotStates = new(StringComparer.OrdinalIgnoreCase);
        private readonly Dictionary<string, DefectCodeDefinition> _defectCodes = new(StringComparer.OrdinalIgnoreCase);
        ...
        /// <summary>
        /// All loaded slot state definitions.
        /// </summary>
        public IReadOnlyCollection<SlotStateDefinition> AllSlotStates => _slotStates.Values;

        /// <summary>
        /// All loaded defect code definitions.
        /// </summary>
        public IReadOnlyCollection<DefectCodeDefinition> AllDefectCodes => _defectCodes.Values;

        /// <summary>
        /// Check if a slot state code is valid.
        /// </summary>
        public bool IsValidSlotState(string code) => _slotStates.ContainsKey(code);

        /// <summary>
        /// Check if a defect code is valid.
        /// </summary>
        public bool IsValidDefectCode(string code) => _defectCodes.ContainsKey(code);

        /// <summary>
        /// Get defect code definition by code string.
        /// </summary>
        public DefectCodeDefinition? GetDefectCodeDefinition(string code) =>
            _defectCodes.TryGetValue(code, out var def) ? def : null;

        public TrayController(TrayControllerConfig config)
        {
            // Load slot state definitions (use defaults if not provided)
            LoadSlotStateDefinitions(config.SlotStates);
            LoadDefectCodeDefinitions(config.DefectCodes);
        ...
        private void LoadSlotStateDefinitions(List<SlotStateDefinition>? definitions)
        {
            _slotStates.Clear();
            ...
            if (definitions != null && definitions.Count > 0)
            {
                foreach (var def in definitions)
                    _slotStates[def.Code] = def;
            }
            else
            {
                // Default built-in slot states (backward compatible with old SlotState enum)
                var defaults = new[]
                {
                    new SlotStateDefinition { Code = "Empty", Category = "State", Description = "Empty slot" },
                    new SlotStateDefinition { Code = "Present", Category = "State", Description = "Slider present, untested" },
                    new SlotStateDefinition { Code = "Pass", Category = "State", Description = "Passed inspection" },
                    new SlotStateDefinition { Code = "Fail", Category = "State", Description = "Failed inspection" },
                    new SlotStateDefinition { Code = "Error", Category = "State", Description = "Inspection error" },
                    new SlotStateDefinition { Code = "Unknown", Category = "State", Description = "Unknown state" }
                };
        ...
        private void LoadDefectCodeDefinitions(List<DefectCodeDefinition>? definitions)
        {
            _defectCodes.Clear();
